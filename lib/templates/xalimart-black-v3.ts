...
// Outer: 600x200 card | Inner: 560x180 shared reference
// Fix: Uses single-table strategy and fixed pixel line-heights for Outlook compatibility.
...
function contactRow(
  ...
  const fontSize = '10px'
  const lineHeight = '16px' // Fixed pixel line-height
  ...
  return `
    <tr>
      <td valign="${isAddress ? 'top' : 'middle'}"
        style="padding:1px 5px 1px 0;width:16px;font-size:0;line-height:0;mso-line-height-rule:exactly;">
        ${simpleIcon(iconSrc, 16)}
      </td>
      <td valign="${isAddress ? 'top' : 'middle'}"
        style="font-size:${fontSize};color:${color};font-family:${FONT};line-height:${lineHeight};padding:1px 0;mso-line-height-rule:exactly;">
        ${content}
      </td>
    </tr>`
}
      ...
      <td align="left" valign="top"
        ...
        <table class="xsig-inner" cellpadding="0" cellspacing="0" border="0" width="560" height="180"
          ...
          <tr height="180" style="height:180px;mso-line-height-rule:exactly;background-color:${BG};">
            ...
            <!-- Col 2: Info (Name, Role, Contacts) — 240px + 20px padding -->
